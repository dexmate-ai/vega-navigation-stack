...
bool usePriorMapProcessing = false;
std::string priorMapPath = "";
const std::string priorMapSourceFrame = "world";
const std::string terrainMapFrame = "map";
pcl::PointCloud<pcl::PointXYZI>::Ptr
    priorMapProcessed(new pcl::PointCloud<pcl::PointXYZI>());
bool priorMapReady = false;
pcl::PointCloud<pcl::PointXYZI>::Ptr
    priorMapRaw(new pcl::PointCloud<pcl::PointXYZI>());
bool priorMapLoaded = false;
...
rclcpp::Node::SharedPtr terrainNode = nullptr;

void tryPreparePriorMap(tf2_ros::Buffer &tfBuffer,
                        const rclcpp::Logger &logger) {
  if (!usePriorMapProcessing || priorMapReady || !priorMapLoaded ||
      priorMapRaw->empty()) {
    return;
  }

  if (!tfBuffer.canTransform(
          terrainMapFrame, priorMapSourceFrame, tf2::TimePointZero,
  ...
  try {
    geometry_msgs::msg::TransformStamped worldToMap =
        tfBuffer.lookupTransform(terrainMapFrame, priorMapSourceFrame,
                                 tf2::TimePointZero);
    Eigen::Isometry3d transformIso =
        tf2::transformToEigen(worldToMap.transform);
    Eigen::Matrix4f transformMat = transformIso.matrix().cast<float>();

    pcl::PointCloud<pcl::PointXYZI>::Ptr priorMapInMap(
        new pcl::PointCloud<pcl::PointXYZI>());
    pcl::transformPointCloud(*priorMapRaw, *priorMapInMap, transformMat);

    pcl::VoxelGrid<pcl::PointXYZI> priorDownSize;
    priorDownSize.setLeafSize(scanVoxelSize, scanVoxelSize, scanVoxelSize);
    priorDownSize.setInputCloud(priorMapInMap);
    priorMapProcessed->clear();
    priorDownSize.filter(*priorMapProcessed);

    priorMapReady = !priorMapProcessed->empty();
    if (priorMapReady) {
      RCLCPP_INFO(logger,
                  "Prior map prepared: %zu input points, %zu after downsampling.",
                  priorMapRaw->size(), priorMapProcessed->size());
    } else {
      RCLCPP_WARN(logger,
                  "Prior map transformation produced an empty result; prior map will be skipped.");
    }
  } catch (const tf2::TransformException &ex) {
    priorMapReady = false;
    RCLCPP_WARN(logger,
                "Failed to transform prior map from %s to %s: %s",
                priorMapSourceFrame.c_str(), terrainMapFrame.c_str(), ex.what());
  }
}
...
void laserCloudHandler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr laserCloud2) {
  ...
  // RCLCPP_INFO(terrainNode->get_logger(), "priorMapReady: %d, usePriorMapProcessing: %d", priorMapReady, usePriorMapProcessing);

  if (priorMapReady) {
    int priorCloudSize = priorMapProcessed->points.size();
    int priorCloudAdded = 0;
    for (int i = 0; i < priorCloudSize; i++) {
      point = priorMapProcessed->points[i];
...
// robot command callback function
...
int main(int argc, char **argv) {
  ...
  nh->declare_parameter<bool>("use_prior_map", usePriorMapProcessing);
  nh->declare_parameter<std::string>("global_map_path", priorMapPath);

  nh->get_parameter("use_prior_map", usePriorMapProcessing);
  nh->get_parameter("global_map_path", priorMapPath);

  std::string registered_scan_topic = "/registered_scan_filtered_map";
  nh->declare_parameter("registered_scan_topic", registered_scan_topic);
  nh->get_parameter("registered_scan_topic", registered_scan_topic);
  ...
  auto subLaserCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>(registered_scan_topic, 5, laserCloudHandler);

  auto subRobotCommand = nh->create_subscription<joy_command_mapper::msg::RobotCommand>("/robot_command", 5, robotCommandHandler);
  ...
  if (usePriorMapProcessing) {
    if (priorMapPath.empty()) {
      RCLCPP_WARN(nh->get_logger(), "use_prior_map enabled but global_map_path is empty; skipping prior map loading.");
      priorMapLoaded = false;
    } else {
      if (pcl::io::loadPCDFile(priorMapPath, *priorMapRaw) < 0) {
        RCLCPP_ERROR(nh->get_logger(), "Failed to load prior map from %s", priorMapPath.c_str());
        priorMapRaw->clear();
        priorMapLoaded = false;
      } else {
        RCLCPP_INFO(nh->get_logger(), "Loaded prior map from %s with %zu points.", priorMapPath.c_str(), priorMapRaw->size());
        priorMapLoaded = true;
      }
    }
  }
  ...
  while (rclcpp::ok()) {
    exec->spin_some();
    tryPreparePriorMap(tfBuffer, nh->get_logger());
    if (newlaserCloud) {
      newlaserCloud = false;
      ...
      terrainCloud2.header.frame_id = terrainMapFrame;
