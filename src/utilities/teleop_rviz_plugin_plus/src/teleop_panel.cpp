  ...
  , mouse_pressed_sent_( false )
{
  ...
  push_button_5_ = new QPushButton( "Clear Terrain Cloud", this );
  layout->addWidget( push_button_5_ );
  ...
  connect( push_button_5_, SIGNAL( pressed() ), this, SLOT( pressButton5() ));
...
{
  if (rclcpp::ok() && reset_publisher_->get_subscription_count() > 0)
  {
...
void TeleopPanel::pressButton2()
{
  if (rclcpp::ok() && robot_cmd_publisher_->get_subscription_count() > 0)
  {
    ...
    robot_cmd.source = "rviz_teleop_plus";
...
{
  if (rclcpp::ok() && robot_cmd_publisher_->get_subscription_count() > 0 && (mouse_pressed_ || mouse_pressed_sent_))
  {
    auto robot_cmd = joy_command_mapper::msg::RobotCommand();
    robot_cmd.header.stamp = node_->now();
    robot_cmd.header.frame_id = "teleop_panel";
    ...
    robot_cmd.vy = angular_velocity_ * 0.5f;
    ...
    robot_cmd.deadman_switch = mouse_pressed_ ? 1.0f : 0.0f;
...
} // end namespace teleop_rviz_plugin_plus

#include <pluginlib/class_list_macros.hpp>
PLUGINLIB_EXPORT_CLASS(teleop_rviz_plugin_plus::TeleopPanel, rviz_common::Panel)
